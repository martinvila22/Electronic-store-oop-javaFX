...
class IntegrationTestingManagerController {
	...
	@Test
	void testControllerManagerCashier() {
    boolean result=managerDash.handleRemoveCashier(cashier.getCashierId());
		
		assertEquals(true,result);//success
		
		result=managerDash.handleRemoveCashier("a123");
		
		assertEquals(false,result);//success
	}
	
	@Test
	void testControllerManagerItem() {
         boolean result=managerDash.handleRemoveProduct(items.get(0).getItemName());
		
		assertEquals(true,result);//success
		
		result=managerDash.handleRemoveProduct("dishwasher");
